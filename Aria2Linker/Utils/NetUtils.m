...
+ (void)doPost:(NSString *)url
    ...
    [manager POST:url parameters:[newDict yy_modelToJSONObject] headers:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (errorData) {
            ...
            if ([serializedData objectForKey:@"result"]) {
                ...
                failure(serializedData[@"error"][@"message"]);
            }
        } else {
            failure(@"服务器无法访问");
        }
...
+ (NSMutableDictionary *)addParameters:(NSDictionary *)dict {
    ...
    [newDict setObject:[NSString stringWithFormat:@"%0.0f", [[NSDate date] timeIntervalSince1970]] forKey:@"t"];
    return newDict;
}

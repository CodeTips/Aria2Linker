...
- (void)viewDidLoad {
    ...
    [self setTitle:NSLocalizedString(@"艾琳", nil)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([_list count] > 0 && indexPath.row != [_list count]) {
        ...
        [APIUtils getVersion:jrs.uri rpcPasswd:jrs.secret
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self isServerCell:indexPath.row]) {
        ...
        [APIUtils getGlobalStatus:jrs.uri rpcPasswd:jrs.secret
    ...
    UITableViewRowAction *deleteRowAction = [UITableViewRowAction
        rowActionWithStyle:UITableViewRowActionStyleDefault
                   ...
                   handler:^(UITableViewRowAction *_Nonnull action, NSIndexPath *_Nonnull indexPath) {
                       ...
                       [[LocalCacheUtils getInstance] setJsonrpcArray:self.list];
...
- (void)addJsonrpcServer {
    ...
    [alertView addAction:[TYAlertAction actionWithTitle:@"确定"
                                                ...
                                                handler:^(TYAlertAction *action) {
                                                    ...
                                                    jrs.secret = [(UITextField *) weakAlertView.textFieldArray[2] text];
                                                    [self.list addObject:jrs];
                                                    [[LocalCacheUtils getInstance] setJsonrpcArray:self.list];
                                                    [self.myTableView reloadData];
                                                }]];
